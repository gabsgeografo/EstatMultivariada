...
# Criar um data frame com os dados de EMD3$conf
df <- data.frame(Dimension1 = -EMD3$conf[,1], Dimension2 = -EMD3$conf[,2], city = cities)

# Plot usando ggplot2
ggplot(df, aes(x = Dimension1, y = Dimension2, label = city)) +
  geom_point(color = "blue", shape = 19) +
  geom_text(size = 3, hjust = 0.5, vjust = -0.5) +
  labs(x = "Dimensão 1", y = "Dimensão 2",
       title = "Mapa das cidades da Ilha Sul da NZ \n Escalonamento Multidimensional Não Métrico usando isoMDS") +
  annotate("text", x = 0.5, y = -0.5, label = paste("Stress = ", round(EMD3$stress, 3)))
